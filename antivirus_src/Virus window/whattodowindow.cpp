...
//adding type of virus name and adress of suspicious file, where it has been located
void WhatToDoWindow::put_adress_and_name_in_susp(char *s1, char *s2)
{
    QString s = QString::fromLocal8Bit(s1,-1);
    //Logger* ff=new Logger();
    adress = new QLabel("Suspicious file has been found\n" + s);
    addr=new QString(s);
    //ff->writeToLog("111");
    strcpy(addr0i,s1);
    s = QString::fromLocal8Bit(s2,-1);
    name = new QLabel(s + " viruses in our database \nuse the same function set!");
    QFont f("Calibri", 30);
    adress->setFont(f);
    name->setFont(f);
}
